...
state setinactive
{
    ...
    {
        llSleep(2);
        llSay(PUBLIC_CHANNEL, "**** Set script inactive ****");
        llSetTimerEvent(1);
        vagent.SendSetScriptRunning(llGetKey(), script_itemid, FALSE);
...
state setactive
{
    state_entry()
    {
        ...
        llSetTimerEvent(1);
        vagent.SendSetScriptRunning(llGetKey(), script_itemid, TRUE);
        if(!vagent.RequestEventQueueGet(eqg_url, 5000))
        {
            llSay(PUBLIC_CHANNEL, "EQG failed");
        }
