...
state giveinventory
{
        ...
        bytearray binarybucket)
    {
        ...
        key originid = inventoryids[rezzableidx];
        ...
        else
        {
            ...
            {
                llSay(PUBLIC_CHANNEL, "CurrentPerms: Mismatch on " + inventorynames[rezzableidx] + ": cur=" + decodeperms(currentperm) + " exp=" + decodeperms(expectedperm));
                success = FALSE;
            }
            
            if(0 != (groupperm & CHECK_PERM))
            {
                llSay(PUBLIC_CHANNEL, "GroupPerms: Mismatch on " + inventorynames[rezzableidx] + ": cur=" + decodeperms(groupperm));
                success = FALSE;
            }
