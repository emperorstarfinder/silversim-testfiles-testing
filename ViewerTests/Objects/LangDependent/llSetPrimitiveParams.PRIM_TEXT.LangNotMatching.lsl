...
default
{
	...
	state_entry()
	{
        llSetPrimitiveParams([PRIM_TEXT, "DefLangName", <1, 1, 1>, 1]);
...
state test1
{
	...
	state_entry()
	{
        ...
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_TEXT, "Hello", <1, 1, 1>, 1]);
		llSetTimerEvent(1.0);
	}
...
state test2
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 2: Setting lang{fr} text to World");
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_TEXT, "World", <1, 1, 1>, 1]);
		llSetTimerEvent(1.0);
	}
