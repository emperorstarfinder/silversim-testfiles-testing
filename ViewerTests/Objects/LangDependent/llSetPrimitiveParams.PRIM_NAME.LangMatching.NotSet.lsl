...
default
{
	...
	state_entry()
	{
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_DESC, "LangDefName"]);
...
state test1
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{default} name to Hello");
        llSetPrimitiveParams([PRIM_NAME, "Hello"]);
...
state test2
{
	...
	state_entry()
	{
        ...
        llSetPrimitiveParams([PRIM_NAME, "World"]);
        vagent.SendRequestObjectPropertiesFamily(0, llGetKey());
		llSetTimerEvent(1.0);
	}
