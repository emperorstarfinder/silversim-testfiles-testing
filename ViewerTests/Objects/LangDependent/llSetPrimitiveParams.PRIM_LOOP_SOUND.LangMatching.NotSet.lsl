...
state test1
{
    ...
    state_entry()
    {
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{default} sound to SOUND_1");
        llSetPrimitiveParams([PRIM_LOOP_SOUND, SOUND_1, 1.0, 0, 10]);
        llSetTimerEvent(1.0);
    }
        ...
        foreach(objdata in objectlist)
        {
            if(objdata.LocalID == localid)
            {
                ...
                if(objdata.LoopedSound != SOUND_1)
                {
                    ...
                    llSay(PUBLIC_CHANNEL, "LoopedSound: " + objdata.LoopedSound);
                    result = FALSE;
                }
                llSetTimerEvent(0);
                state test2;
...
state test2
{
    ...
    state_entry()
    {
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{default} sound to SOUND_2");
        llSetPrimitiveParams([PRIM_LOOP_SOUND, SOUND_2, 1.0, 0, 10]);
        llSetTimerEvent(1.0);
    }
