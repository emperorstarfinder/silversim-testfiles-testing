...
default
{
	...
	state_entry()
	{
        llSetPrimitiveParams([PRIM_TEXTURE_ANIM, ANIM_ON | LOOP, ALL_SIDES, 2, 2, 0, 4, 1]);
...
state test1
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{fr} text to Hello");
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_TEXTURE_ANIM, ANIM_ON | LOOP, ALL_SIDES, 4, 4, 0, 4, 1]);
		llSetTimerEvent(1.0);
	}
	...
	objectupdate_received(agentinfo agent, float timedilation, objectdatalist objectlist)
	{
        foreach(objdata in objectlist)
        {
            ...
            {
                received = TRUE;
                if(objdata.TextureAnim.SizeX != 4 || objdata.TextureAnim.SizeY != 4)
                {
...
state test2
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{fr} text to World");
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_TEXTURE_ANIM, ANIM_ON | LOOP, ALL_SIDES, 8, 8, 0, 4, 1]);
		llSetTimerEvent(1.0);
	}
	...
	objectupdate_received(agentinfo agent, float timedilation, objectdatalist objectlist)
	{
        foreach(objdata in objectlist)
        {
            ...
            {
                received = TRUE;
                if(objdata.TextureAnim.SizeX != 8 || objdata.TextureAnim.SizeY != 8)
                {
...
state test3
{
	...
	objectupdate_received(agentinfo agent, float timedilation, objectdatalist objectlist)
	{
        foreach(objdata in objectlist)
        {
            ...
            {
                received = TRUE;
                if(objdata.TextureAnim.SizeX != 2 || objdata.TextureAnim.SizeY != 2)
                {
