...
const key SOUND_DEFLANG = "4c8c3c77-de8d-bde2-b9b8-32635e0fd4a6";
...
default
{
	...
	state_entry()
	{
        llSetPrimitiveParams([PRIM_LOOP_SOUND, SOUND_DEFLANG, 1.0, 0, 10]);
...
state test1
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{fr} sound to SOUND_1");
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_LOOP_SOUND, SOUND_1, 1.0, 0, 10]);
		llSetTimerEvent(1.0);
	}
	...
	objectupdate_received(agentinfo agent, float timedilation, objectdatalist objectlist)
	{
        foreach(objdata in objectlist)
        {
            ...
            {
                received = TRUE;
                if(objdata.LoopedSound != SOUND_1)
                {
...
state test2
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{fr} sound to SOUND_2");
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_LOOP_SOUND, SOUND_2, 1.0, 0, 10]);
		llSetTimerEvent(1.0);
	}
	...
	objectupdate_received(agentinfo agent, float timedilation, objectdatalist objectlist)
	{
        foreach(objdata in objectlist)
        {
            ...
            {
                received = TRUE;
                if(objdata.LoopedSound != SOUND_2)
                {
...
state test3
{
	...
	objectupdate_received(agentinfo agent, float timedilation, objectdatalist objectlist)
	{
        foreach(objdata in objectlist)
        {
            ...
            {
                received = TRUE;
                if(objdata.LoopedSound != SOUND_DEFLANG)
                {
