...
state test1
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{default} touchtext to Hello");
        llSetPrimitiveParams([PRIM_TOUCH_TEXT, "Hello"]);
    ...
    objectproperties_received(agentinfo agent, objectpropertieslist objlist)
	{
        foreach(data in objlist)
        {
            ...
            {
                received = TRUE;
                if(data.TouchText != "Hello")
                {
                    llSay(PUBLIC_CHANNEL, "Test 1: failed");
                    llSay(PUBLIC_CHANNEL, "TouchText: " + data.TouchText);
                    result = FALSE;
                }
...
state test2
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 2: Setting lang{default} touchtext to World");
        llSetPrimitiveParams([PRIM_TOUCH_TEXT, "World"]);
        vagent.SendObjectSelect([localid]);
		llSetTimerEvent(2.0);
	}
