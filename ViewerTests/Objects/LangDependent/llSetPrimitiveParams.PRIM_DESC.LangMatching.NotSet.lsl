...
state test1
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{default} desc to Hello");
        llSetPrimitiveParams([PRIM_DESC, "Hello"]);
...
state test2
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 2: Setting lang{default} desc to World");
        llSetPrimitiveParams([PRIM_DESC, "World"]);
        vagent.SendRequestObjectPropertiesFamily(0, llGetKey());
		llSetTimerEvent(1.0);
	}
