...
default
{
	...
	state_entry()
	{
        llSetPrimitiveParams([PRIM_DESC, "DefLangName"]);
...
state test1
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 1: Setting lang{fr} desc to Hello");
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_DESC, "Hello"]);
    ...
    objectpropertiesfamily_received(agentinfo agent, integer requestFlags, key objectid, objectpropertiesfamilydata data)
	{
        ...
        if(objectid != llGetKey() ||
            data.Description != "DefLangName")
        {
            llSay(PUBLIC_CHANNEL, "Test 1: failed");
            llSay(PUBLIC_CHANNEL, "Desc: " + data.Description);
            result = FALSE;
        }
        llSetTimerEvent(0);
        state test2;
...
state test2
{
	...
	state_entry()
	{
        llSay(PUBLIC_CHANNEL, "Test 2: Setting lang{fr} desc to World");
        llSetPrimitiveParams([PRIM_LANGUAGE, "fr", PRIM_DESC, "World"]);
        vagent.SendRequestObjectPropertiesFamily(0, llGetKey());
		llSetTimerEvent(1.0);
	}
