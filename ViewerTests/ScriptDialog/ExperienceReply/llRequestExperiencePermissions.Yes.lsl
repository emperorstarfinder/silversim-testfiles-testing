...
state test
{
	...
	{
		if(target_id != agentid)
		{
			llSay(PUBLIC_CHANNEL, "Unexpected target_id. Got " + target_id);
			result = FALSE;
		}
	...
	{
		llSay(PUBLIC_CHANNEL, "got experience_permissions_denied");
		result = FALSE;
	}
	...
	timer()
	{
		...
		if(respcount == 0)
		{
			llSay(PUBLIC_CHANNEL, "No experience_permissions");
			result = FALSE;
		}
		state logout;
	}
