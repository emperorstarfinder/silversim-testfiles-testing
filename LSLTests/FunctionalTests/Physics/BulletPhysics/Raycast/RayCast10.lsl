...
state raycast1
{
	state_entry()
	{
		...
		list result = llCastRay(objectpos - (targetpos - objectpos), targetpos + (targetpos - objectpos), [RC_DATA_FLAGS, RC_GET_LINK_NUM | RC_GET_ROOT_KEY]);
...
state raycast2
{
	state_entry()
	{
		...
		/* format rules [<id>, <link>*, <position>, <normal>*] */
		list result = llCastRay(targetpos + (targetpos - objectpos), objectpos - (targetpos - objectpos), [RC_DATA_FLAGS, RC_GET_LINK_NUM | RC_GET_ROOT_KEY]);
