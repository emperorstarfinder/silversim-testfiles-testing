...
default
{
	state_entry()
	{
		...
		llSleep(1);
		integer before2 = llList2Integer(btGetPrimParams(llGetKey(), [BULLET_PARAM_ACT_PHYSICS_SHAPE_UPDATE_SERIAL]), 0);
		...
		integer after2 = llList2Integer(btGetPrimParams(llGetKey(), [BULLET_PARAM_ACT_PHYSICS_SHAPE_UPDATE_SERIAL]), 0);
		
		llSay(PUBLIC_CHANNEL, "CUR_PARAMETER_UPDATE_SERIAL before: " + (string)before);
		llSay(PUBLIC_CHANNEL, "ACT_PARAMETER_SHAPE_SERIAL before: " + (string)before2);
		llSay(PUBLIC_CHANNEL, "CUR_PARAMETER_UPDATE_SERIAL after: " + (string)after);
		llSay(PUBLIC_CHANNEL, "ACT_PARAMETER_SHAPE_SERIAL after: " + (string)after2);
