...
//
//          This is the main vendor script with floattext
//
...
SetCurrentItem(integer item)
{
    currentItem = item;
    string hoverText = "";
    if (llList2String(FREE_OPTIONAL_ITEMS,currentItem) == "")
    {
        hoverText = "Folder "+(string)(currentItem + 1)+" of 9\n"
                    +llList2String(FOLDER_NAMES,currentItem)+" - L$ "+(string)llList2String(FOLDER_PRICES,currentItem);
    }
    else
    {
        hoverText = "Folder "+(string)(currentItem + 1)+" of 9\n"
                    +llList2String(FOLDER_NAMES, currentItem)+" - L$ "+(string)llList2String(FOLDER_PRICES, currentItem)+"\n"
                    +"Touch large panel for a notecard";
    }
    llSetText(hoverText,<1.0,1.0,1.0>,(float)TRUE);
...
state disabled
{
    ...
    state_entry()
    {
        ...
        llSetText("Vendor offline!",<1,0,0>,(float)TRUE);
        llSetLinkPrimitiveParamsFast(LINK_THIS,[
    ...
    state_exit() { llSetText("",ZERO_VECTOR,(float)FALSE); }
}