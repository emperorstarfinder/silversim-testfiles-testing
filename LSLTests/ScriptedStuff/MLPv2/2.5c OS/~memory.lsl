...
string  Dashes = "_________________________\n";
...
add_pose(string name, string data) {
    if (name == "MEMORY") {
        Memory = (integer) llGetSubString(data, 1, -2);
        return;
    }
...
state on {
    ...
    link_message(integer from, integer num, string str, key dkey) {
        ...
        if (str == "OK?") {                                 //question from menu, before loading menu
            ...
            integer ix;
            for (ix = 0; ix < PosCount; ++ix) {
                string xname = llList2String(Poses, ix);
                llOwnerSay("{" + xname + "} " + get_pose_data(xname));
            }
            ...
            if (llGetSubString((string)dkey, 0, 0) == "<") {    //SAVE
                ...
                string data = (string)dkey;
                integer ix = llListFindList(Poses, [str]);
                if (ix == -1) {
                    add_pose(str, data);
                } else {
                    store_pose(data, ix);
