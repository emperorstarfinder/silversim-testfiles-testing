...
integer Running;
integer ListenId;
integer Memory;         // optional memory limit for llSetMemoryLimit()
...
doMenu(integer inhibit_showing) {
    ...
    bix = llList2Integer(buttonindex, menu+1);        //position of first button for next (sub)menu
    ...
    list bbuttons = llList2List(buttons, b0, bix - 1);
    if (SaneMenuOrder) {
        bbuttons =
...
integer handle_cmd(string button, integer sequenced) {
   integer ix = llSubStringIndex(button, "[PUSH] ");
    ...
    bix = llListFindList(buttons, (list) button);                     //find position of cmd
    ...
    string cmd = llList2String(commands,bix);                         //get command
    ...
    if (cmd == "TOMENU") {
        ...
        integer ixx = llList2Integer(users, newmenu); 
        if (user == owner || (ixx == 1 && group) || ixx == 2) {         //0=owner 1=group 2=all
            MenuStack += (list)menu;
            MenuPage = 0;
            menu = newmenu;
            doMenu(sequenced);
        ...
        llOwnerSay("~menu: " + (string)llGetFreeMemory() + " bytes free");
        llMessageLinked(LINK_THIS, Adjusting, "MEMORY", NullKey);
    } else if (cmd == "HIDE") {
        setBalls("0");
    } else if (cmd == "SHOW") {
        setBalls("SHOW");
...
default {
    ...
    link_message(integer from, integer num, string str, key id) {
        ...
        if (num == -1) {
            ...
            Memory          = (integer) llList2String(args,6);
