...
doMenu(integer inhibit_showing) {
    ...
    list buttons = llList2List(buttons, b0, bix - 1);
    if (SaneMenuOrder) {
        buttons =
...
integer handle_cmd(string button, integer sequenced) {
   integer ix = llSubStringIndex(button, "[PUSH] ");
    ...
    if (cmd == "TOMENU") {
        ...
        integer ix = llList2Integer(users, newmenu); 
        if (user == owner || (ix == 1 && group) || ix == 2) {         //0=owner 1=group 2=all
