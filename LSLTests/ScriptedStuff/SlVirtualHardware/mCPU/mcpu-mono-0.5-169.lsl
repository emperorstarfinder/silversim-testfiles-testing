...
// *** CONSTANTS ***
integer digits = 14; // machine bus width, bits
float frequency = 2.5; // synchronous clock timer frequency, Hz
integer drop_for_all = 0; // accept program notecards from non-owners
integer m_debug = 1; // default debug setting
string ver = "0.5 rev 169"; // version information
string ledsprefix = "ledbit"; // name prefix for digits lamps
integer mem_local_top = 256; // 2^8 words (optimal value for main CPU object)
integer memory_channel = -423531; // external memory channel
integer cache_size = 8; // L1+2 cache table size, records
integer ERAM_Max_Requests = 51; // max amount of unanswered external RAM requests
...
update_summator_leds()
{
    ...
    for (i=digits-1; i>=0; i--) {
        j = (integer)llPow(2,i);
        if (k >= j) {
//            set_led_state(ledsprefix+(string)i,1);
            summfast += [1];
            k -= j;
        } else {
//            set_led_state(ledsprefix+(string)i,0);
            summfast += [0];
        }
    }
    // fast update algorithm (added on rev 161)
    string sk;
    j = llGetNumberOfPrims();
    for (i=1; i<=j; i++) {
        sk = llGetLinkName(i);
        if (llGetSubString(sk,0,5) == ledsprefix) {
            k = (integer)llGetSubString(sk,6,-1);
            if ((k >= 0) && (k < digits))
                llSetLinkPrimitiveParamsFast(i,[PRIM_GLOW,ALL_SIDES,(float)llList2Integer(summfast,(digits-1-k))]);
        }
    }
}
...
{
    if ((RAM_FLG % 2 > 0) && (RAM_rg < 0)) {
        // deadlock blocker (or maybe spawner) ;-}
        RAM_rqcnt++;
        debug("RQ "+(string)RAM_rqcnt);
        if (RAM_rqcnt > ERAM_Max_Requests) {
            llSay(0,"External RAM fatal error: chip not responding!");
            llSleep(1.0);
            llResetScript();
        }
        return -1;
...
say_time_stats()
{
    integer secs = llGetUnixTime() - start_time;
    float fps = (float)cnt_cycles / (float)(secs);
    llSay(0,"Time stats: "+(string)cnt_cycles+" cycles within "+(string)secs+" seconds");
    llSay(0,(string)fps+" CPS");
}
...
default
{
    ...
    {
        if ((ownerlock) && (llDetectedKey(0) != owner)) return;
        if (w_flag) {
            poweron(0);
            llSay(0,"Stopped.");
            say_time_stats();
...
//        if (_id == llGetKey()) return;
        ...
        if (_msg=="reset") llResetScript();
            ...
            // TODO: write some :)
            llSay(0,"debug getsum vcache rawcache 'cache reset' oneshot ownerlock ");
        } else if (_msg == "debug") {
            ...
            else llSay(0,"Normal mode");
        } else if (_msg == "ownerlock") {
            ownerlock = 1 - ownerlock;
            if (ownerlock) llSay(0,"Locked");
            else llSay(0,"Unlocked");
...
//                memory = llListReplaceList(memory,lex,IP,IP);
                IP++;
                if (prog_datapoint < IP)
                    // COLLIDE!
                    card_read_err_handler("Memory space collision detected at $"+(string)IP);
            }
